...
### 1. Permanent Default Supabase Cloud Connection & Personal Access Gate
...
- **Tailored Personal Habit Tracker Experience**:
  - As requested, since Vignesh is the primary user and owner, the application opens in unlocked editing mode by default.
  - All habit toggles, mood entries, daily reflections, and goal adjustments persist immediately without requiring repeated PIN unlocks or session timeouts.
  - The client automatically attaches pre-authorized credentials (`9500`) and the server accepts write mutations directly to Supabase cloud.
  - If a 401 response ever occurs due to an expired token, the API client automatically recovers using the owner access gate (`9500`) and retries seamlessly in the background without dropping changes.
- **Elimination of "Unable to Save" False Errors & Multi-Attempt Network Resilience**:
  - Replaced generic connection error masks with accurate, transparent status reporting and self-healing auto-retry logic.
  - Automatic exponential backoff retry (up to 3 attempts with 350ms/700ms delays) for momentary network drops, browser reconnection delays, or server warmup/cold-start periods (HTTP 502/503/504), preventing temporary glitches from showing frightening error messages.
...
- **On-Demand Lock / Unlock Toggle**:
  - If Vignesh wishes to temporarily place the tracker into read-only mode (for example, when sharing his screen), he can click the compact Lock icon (`#header-view-only-indicator` / `#header-edit-unlocked-indicator`) in the header.
  - Entering the personal 4-digit PIN `9500` instantly unlocks full editing capabilities again.
  - **4-Digit Visual Slots**: Four animated digit slots displaying masked bullets or visible numbers with live focus rings and auto-verification upon entering the 4th digit.
  - **Touch & Hardware Keyboard Input**: Full support for physical keyboard typing (0-9, Backspace, Delete, Escape, Enter) and on-screen numeric keypad.
...
- **Core Modules**:
  ...
  4. **Mood Calendar (`MoodCalendarView.tsx`)**:
     ...
     - Public viewing without PIN; modifications require PIN `321`.
  5. **Mood Analytics (`MoodAnalyticsView.tsx`)**:
   ...
   6. **Edit & Delete Modals (`EditMoodModal.tsx`, `DeleteMoodModal.tsx`)**:
      - Secure dialogs requiring personal PIN (`0321` or `321`) to safeguard personal reflections while keeping visual viewing public.
...
- **Security & PIN Access**: Personal 4-digit PIN gate (`0321` / `321`) with support for a "Continue as Visitor" view-only preview mode and auto-lock inactivity timer.
...
- `POST /api/auth/verify-password`: Password/4-digit PIN verification (`0321` or `321`) returning an edit session token (`token`, `expiresIn`).
- `POST /api/auth/lock`: Revokes active edit session token.
- `POST /api/auth/verify-pin`: PIN verification endpoint.
- `GET /api/moods` & `POST /api/moods`: Fetch all mood logs and save/upsert a mood entry with automated synchronization to `daily_journal` for persistent cloud storage.
- `PUT /api/moods/:id` & `DELETE /api/moods/:id`: Update and delete individual mood entries (PIN-protected).
...
# Personal Edit Protection Password / PIN (Default: 9500)
APP_EDIT_PASSWORD="9500"
```
